...
Deploy CoSeal on Kubernetes with production-ready defaults.

## Quick Start

```bash
# Add the chart repository (when published)
helm repo add coseal https://charts.coseal.dev

# Install with custom values
helm install coseal coseal/coseal \
  --namespace coseal \
  --create-namespace \
...
helm install coseal deploy/helm/coseal/ \
  ...
  --set database.url="postgresql://coseal:password@postgres:5432/coseal" \
...
| `storage.bucket` | S3 bucket for documents | `coseal-documents` |
...
```bash
helm upgrade coseal coseal/coseal \
  --namespace coseal \
  -f values-production.yaml
```
...
```bash
helm uninstall coseal --namespace coseal
```
