...
import getSigningUrl from '@salesforce/apex/CoSealEnvelopeController.getSigningUrl';
...
export default class CosealSigningEmbed extends LightningElement {
    ...
    handleMessage(event) {
        // Handle postMessage from CoSeal signing iframe
        if (event.data && event.data.type === 'coseal:signing-complete') {
        ...
        } else if (event.data && event.data.type === 'coseal:signing-declined') {
