# /coseal:send — Send for Signature
...
- Provide the envelope ID for tracking: "Use `/coseal:status {envelopeId}` to check progress"

## Error Handling
- If the document is not a PDF, inform the user CoSeal currently only supports PDFs.
...
4. Sends envelope
5. Responds: "✓ Sent `nda.pdf` for signature. Alice and Bob will receive signing links via email. Envelope ID: `env_abc123`. Use `/coseal:status env_abc123` to track progress."
